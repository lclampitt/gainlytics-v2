  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const v2 = `https://world.openfoodfacts.org/api/v2/search?search_terms=${encodeURIComponent(q)}&page_size=20&fields=${fields}`;
        let data;
        try {
          const res = await fetch(v2);
          data = await res.json();
        } catch {
          const legacy = `https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(q)}&json=true&page_size=20`;
